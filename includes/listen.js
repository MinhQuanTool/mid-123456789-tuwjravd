  ...
  setInterval(async() => {
   ...
   const timechecktt = moment.tz('Asia/Ho_Chi_Minh').format('DD/MM/YYYY || HH:mm:ss'); 
    const haha = `\n────────────────────\n💬 Tổng tin nhắn: ${storage.reduce((a, b) => a + b.count, 0)}\n⏰ Time: ${timechecktt}\n✏️ Các bạn khác cố gắng tương tác nếu muốn lên top nha`;    
        ...
        api.sendMessage({body:checkttBody + haha,attachment: (await require("axios").get(global.api("vdanime", "datajson"), { responseType: "stream" })).data},   checkttFile.replace('.json', ''), (err) => err ? console.log(err) : '');
    ...
    const tctt = moment.tz('Asia/Ho_Chi_Minh').format('DD/MM/YYYY || HH:mm:ss');
     ...
     api.sendMessage({body:checkttBody + dzvcl,attachment: (await require("axios").get(global.api("vdcos", "datajson"), { responseType: "stream" })).data}, checkttFile.replace('.json', ''), (err) => err ? console.log(err) : '');
...
if (event.logMessageType === "log:thread-icon") {
    let read = await fs.readFile('modules/commands/data/antiemoji.json', 'utf-8');
    let antiData = read ? JSON.parse(read) : {};

    // Tìm thông tin emoji của threadID
    let threadEntry = antiData[event.threadID];
    ...
    if (threadEntry) {
        ...
        if (findAd || botID.includes(event.author)) {
            ...
            await fs.writeFile('modules/commands/data/antiemoji.json', JSON.stringify(antiData, null, 4), 'utf-8');
...
switch (event.type) {
    ...
    api.shareContact(msg + khanh, event.author,event.threadID, async (err, info) => {
